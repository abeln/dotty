...
class SymbolicXMLBuilder(parser: Parser, preserveWS: Boolean)(implicit ctx: Context) {
  ...
  def comment(span: Span, text: String): Tree  = atSpan(span)( Comment(asInstanceOfString(const(text))) )
  def charData(span: Span, txt: String): Tree  = atSpan(span)( makeText1(asInstanceOfString(const(txt))) )

  def procInstr(span: Span, target: String, txt: String): Tree =
    atSpan(span)( ProcInstr(asInstanceOfString(const(target)), asInstanceOfString(const(txt))) )
