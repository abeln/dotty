...
object Hashable {
  ...
  class Binders(val tp: BindingType, val next: Nullable[Binders])

  /** A null terminated list of pairs of BindingTypes. Used for isomorphism tests. */
  class BinderPairs(tp1: BindingType, tp2: BindingType, next: Nullable[BinderPairs]) {
...
trait Hashable {
  ...
  final def typeHash(bs: Nullable[Binders], tp: Type): Int =
    if (bs == null || tp.stableHash) tp.hash else tp.computeHash(bs)

  def identityHash(bs: Nullable[Binders]): Int = avoidSpecialHashes(System.identityHashCode(this))

  protected def finishHash(bs: Nullable[Binders], seed: Int, arity: Int, tp: Type): Int = {
    val elemHash = typeHash(bs, tp)
    if (elemHash == NotCached) return NotCached
    finishHash(hashing.mix(seed, elemHash), arity + 1)
  }

  protected def finishHash(bs: Nullable[Binders], seed: Int, arity: Int, tp1: Type, tp2: Type): Int = {
    val elemHash = typeHash(bs, tp1)
    if (elemHash == NotCached) return NotCached
    finishHash(bs, hashing.mix(seed, elemHash), arity + 1, tp2)
  }

  protected def finishHash(bs: Nullable[Binders], seed: Int, arity: Int, tps: List[Type]): Int = {
    var h = seed
    var xs = tps
    var len = arity
    while (!xs.isEmpty) {
  ...
  protected def finishHash(bs: Nullable[Binders], seed: Int, arity: Int, tp: Type, tps: List[Type]): Int = {
    val elemHash = typeHash(bs, tp)
    if (elemHash == NotCached) return NotCached
  ...
  protected final def doHash(bs: Nullable[Binders], tp: Type): Int =
    finishHash(bs, hashSeed, 0, tp)

  protected final def doHash(bs: Nullable[Binders], x1: Any, tp2: Type): Int =
    finishHash(bs, hashing.mix(hashSeed, x1.hashCode), 1, tp2)

  protected final def doHash(bs: Nullable[Binders], tp1: Type, tp2: Type): Int =
    finishHash(bs, hashSeed, 0, tp1, tp2)

  protected final def doHash(bs: Nullable[Binders], x1: Any, tp2: Type, tp3: Type): Int =
    finishHash(bs, hashing.mix(hashSeed, x1.hashCode), 1, tp2, tp3)

  protected final def doHash(bs: Nullable[Binders], tp1: Type, tps2: List[Type]): Int =
    finishHash(bs, hashSeed, 0, tp1, tps2)

  protected final def doHash(bs: Nullable[Binders], x1: Any, tp2: Type, tps3: List[Type]): Int =
